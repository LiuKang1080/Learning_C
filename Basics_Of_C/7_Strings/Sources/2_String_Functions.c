// General Purpose String Functions in C

/*
General Purpose String Functions in C:
    - Most of the general string functions are located in the string.h header file.
    - In order to use these functions we must #include the <string.h> header file.
...
Copying Strings:
    ...
    - The strcpy() function does NOT check to see if the source string actually fits into the target string. 
    - Generally strncpy() is the safer way to copy strings. strncpy() takes in 3 arguments - the 3rd argument specifies the 
        maximum number of characters to copy, specifying how much of the string we want to copy.
...
String Concatenation:
    - We use the strcat() funcation. Takes in 2 strings as arguments, takes a copy of the second string and attaches it to the
        end of the first string, the combined version becomes the first string. The second string is not altered.

    - Returns the value of its first argument, the address of the first character of the string to which the second string is 
        appended.

    - If there is not enough space in the first array, we can run into buffer overflow problems. Like strcpy() and strncpy() 
        strcat() also has strncat() function. Takes another argument indicating the maximum number of characters to add. It will
        continue to add character until it reaches the number specified or until it reaches the Null terminator, whichever one 
        comes first.
