...
Using the const keyword with pointers. 
    ...
    - since we declared the pointer to be a const the compiler will check for any statements that attempt to change the value pointed to by
      p_value pointer and flag all statements as errors.
