...
/*
Pointers, Char Arrays, and Strings:
...
int string_length(const char *pointer) {
    /*
        ...
        - We need to maintin const-ability whenever we can. When we're creating our local pointer if we do not create it as a 
            const then we are casting away the const. Generally if a pointer is pointing to something that is a const, then we 
            should create the pointer as a const pointer.
    */
