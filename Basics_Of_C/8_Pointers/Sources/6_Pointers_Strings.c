...
/*
    ...
    - to point to the previous element of the array we can use the decrement operator: --p_text (We need to be sure that we do not use this operator
        at the beginning of the array, we will get an Out Of Bounds Error.) 

    - While Loop with pointers:

        void copy_string(char *to, char *from) {
            while (*from) {                         // the NULL terminator is equal to 0, the while loop will end when it reaches the NULL terminator
                *to++ = *from++;                    // pay attention to where the ++ is, the incrementation happens AFTER this expression is evaluated
            }
...
int main() {
    /*
        ...
        1) Write a function that calculates the length of a string.
            ...
            - The function should subtract 2 pointers, 1 pointing to the end of the string, the other pointing to the beginning of the string.
