...
/*
    ...
    Pass by Value = is when a function copies the actual value of an argument into the formal parameter of the function. Changes
                made to the parameter inside the function have no effect on the argument.
        - ex) passing in a int or double variable into the parameter of a function the variable outside does not change, because 
                it is copied.
